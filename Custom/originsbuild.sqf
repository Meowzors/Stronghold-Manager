...
switch(_type) do {
	case 'B1' : {
		...
		if(_playerUID in owner_B1 or _playerUID in owner_H1) exitwith {closeDialog 1; _abort = true; _message = format["You have already built a Level 1 House!", _name];};
	...
	case 'B2' : {
		...
		if(_humanity > _limit) exitwith {closeDialog 1; _abort = true; _message = format["You need at least %1 Humanity to Build the %2!",_limit,_name];};
		if(!(_playerUID in owner_B1 or _playerUID in owner_H1)) exitwith {closeDialog 1; _abort = true; _message = "You need to Build a Level 1 House first!";};
		if(_playerUID in owner_B2 or _playerUID in owner_H2) exitwith {closeDialog 1; _abort = true; _message = format["You have already built a Level 2 House!", _name];};
	};
	case 'B3' : {
		...
		if(_humanity > _limit) exitwith {closeDialog 1; _abort = true; _message = format["You need at least %1 Humanity to Build the %2!",_limit,_name];};
		if(!(_playerUID in owner_B2 or _playerUID in owner_H2)) exitwith {closeDialog 1; _abort = true; _message = "You need to Build a Level 2 House first!";};
		if(_playerUID in owner_B3 or _playerUID in owner_H3) exitwith {closeDialog 1; _abort = true; _message = format["You have already built a Level 3 House!", _name];};
	};
	case 'H1' : {
	...
	case 'SG' : {
		...
		_price = 1;
		_name = "Small Garage";
		if(!(_playerUID in owner_B1 or _playerUID in owner_H1)) exitwith {closeDialog 1; _abort = true; _message = format["You must build a level 1 house first!", _name];};
		if(_playerUID in owner_SG) exitwith {closeDialog 1; _abort = true; _message = format["You have already built your %1!", _name];};
	};
	case 'LG' : {
		_classname = "velkaGaraz";
		_price = 2;
		_name = "Large Garage";
		if(!(_playerUID in owner_B3 or _playerUID in owner_H3)) exitwith {closeDialog 1; _abort = true; _message = format["You must build a level 3 house first!", _name];};
		if(!(_playerUID in owner_SG)) exitwith {closeDialog 1; _abort = true; _message = format["You must build a small garage first!", _name];};
		if(_playerUID in owner_LG) exitwith {closeDialog 1; _abort = true; _message = format["You have already built your %1!", _name];};
	};
	case 'PG' : {
		_classname = "kingramida";
		_price = 5;
		_name = "Pyramid Garage";
		if(!(_playerUID in owner_LG)) exitwith {closeDialog 1; _abort = true; _message = format["You must build a Large Garage first!", _name];};
		if(_playerUID in owner_PG) exitwith {closeDialog 1; _abort = true; _message = format["You have already build your %1!", _name];};
	};
	case 'krepost' : {
		_classname = "krepost";
		_price = 10;
		_offset = [0,20,7.5];
		_name = "Stronghold";
		if(!(_playerUID in owner_B3 or _playerUID in owner_H3)) exitwith {closeDialog 1; _abort = true; _message = "You need to Build a Level 3 House first!";};
	};
};
...
_object setVectorUp surfaceNormal position _object;  //align house with ground
...
if(sectB) then {_cancel = true; _reason = "Cannot build in SectorB."; };
if(!_cancel) then {
...
_tmpbuilt = createVehicle [_classname, _location, [], 0, "CAN_COLLIDE"];
 _tmpbuilt setVectorUp surfaceNormal position _tmpbuilt;  //align house with ground
...
_tmpbuilt setVariable ["PlayerName",_playerName,true];
...
switch (_type) do {
	...
	case 'PG' : {
		player removeWeapon "EvPhoto";
		owner_PG set [count owner_PG, _playerUID];
...
if(_classname == "krepost") then {
	PVDZE_obj_Publish = [dayz_characterID,_tmpbuilt,[_dir,_location,_playerUID,_playerName,"[]"],_classname];
	publicVariableServer "PVDZE_obj_Publish";
	cutText ["You are now a stronghold owner! You can now add access for your friends if you want.","PLAIN",3];
} else {
	_dir = getDir player;
	PVDZE_obj_Publish = [dayz_characterID,_tmpbuilt,[_dir,_location,_playerUID,_playerName],_classname];
	publicVariableServer "PVDZE_obj_Publish";
